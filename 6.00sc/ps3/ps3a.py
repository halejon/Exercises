...
def get_word_score(word, n):
    ...
    #Is this an appropriate use of a global variable / dictionary?
    #Is this an appropriate use of True / False being overloaded with 1/0 values?
    return sum([SCRABBLE_LETTER_VALUES[letter] for letter in list(word)]) * len(word) + 50 * (n == len(word))
...
def is_valid_word(word, hand, word_list):
    ...
    """
    #Need to test that the word contains letters only from the hand and it is in the word list
    #Can do some sort of iteration across the word / hand to see if we get negative values and return false
    #link logic with an AND statement
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while hand:
        hand_list = [' '.join(letter * hand[letter]) for letter in hand.keys()]
...
def play_game(word_list):
    ...
    * If the user inputs anything else, ask them again.
    ...
    while game_state <> "e":
        game_state = raw_input("Please enter 'n' for a new(random) hand, 'r' to replay the last hand, or 'e' to exit the game")

        if game_state == 'e':
            break
        elif game_state == 'r':
            current_hand = play_hand(current_hand, word_list)
        else:
            current_hand = play_hand(deal_hand(HAND_SIZE),word_list)
